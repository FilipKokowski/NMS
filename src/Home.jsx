...
const InputField = ({type, id, style, placeholder,}) => {
  ...
  return (type == 'textarea') ? <textarea id={id} style={style} placeholder={placeholder}></textarea> : <input defaultValue={(type === 'date' ? date : "")} type={type} id={id} style={style} placeholder={placeholder}></input>;
}
    ...
    <div style={style}>
      <div style={{width: '100%', height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center'}}>
        ...
        <div style={{display: 'flex', flexDirection: 'column', alignItems: 'center'}}>
          ...
          <InputField id={'description'} type='textarea' style={{width: '30vh', height: '20vh', borderRadius: '1.5vh', border: 'none', backgroundColor: '#444444'}}/>
        </div>
...
const Home = () => {
  ...
  return (
    <div>
      ...
      <div style={{width: '100vw', height: '25vh', overflowY: 'scroll', display: 'flex', justifyContent: 'center', alignItems: 'baseline'}}>
        <table style={{fontSize: '1.5vh'}}>
          <tbody>
            ...
            {state.favourites.map((elem) => { return <tr key={uuidv4()} style={{backgroundColor: (new Date().toJSON().slice(0,10).replace(/-/g,'-') == elem.date ? 'red' : '')}}><th>{elem.date}</th><th>{elem.type}</th><th>{elem.contractor}</th><th>{elem.description}</th></tr>})}
          </tbody>
        </table>
      </div>
    </div>
